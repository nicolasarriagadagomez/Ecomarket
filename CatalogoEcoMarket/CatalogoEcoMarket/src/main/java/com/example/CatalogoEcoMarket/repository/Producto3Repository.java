package com.example.CatalogoEcoMarket.repository;

import java.util.ArrayList;
import java.util.List;

import org.springframework.stereotype.Repository;

import com.example.CatalogoEcoMarket.model.Producto3;

@Repository
public class Producto3Repository {

    private List<Producto3> listaProductos = new ArrayList<>();

    public List<Producto3> obtenerProductos() {
        return listaProductos;
    }

    public Producto3 buscarPorId(int id) {
        for (Producto3 producto : listaProductos) {
            if (producto.getId() == id) {
                return producto;
            }
        }
        return null;
    }

    public Producto3 buscarPorCodigo(String codigo) {
        for (Producto3 producto : listaProductos) {
            if (producto.getCodigo().equals(codigo)) {
                return producto;
            }
        }
        return null;
    }

    public Producto3 guardar(Producto3 prod) {
        listaProductos.add(prod);
        return prod;
    }

    public Producto3 actualizar(Producto3 prod) {
        int posicion = -1;
        for (int i = 0; i < listaProductos.size(); i++) {
            if (listaProductos.get(i).getId() == prod.getId()) {
                posicion = i;
                break;
            }
        }
        if (posicion == -1) {
            return null; // no existe producto para actualizar
        }
        Producto3 productoActualizado = new Producto3();
        productoActualizado.setId(prod.getId());
        productoActualizado.setCodigo(prod.getCodigo());
        productoActualizado.setNombre(prod.getNombre());
        productoActualizado.setMarca(prod.getMarca());
        productoActualizado.setPrecio(prod.getPrecio());
        productoActualizado.setStock(prod.getStock());
        productoActualizado.setDescripcion(prod.getDescripcion());
        
        listaProductos.set(posicion, productoActualizado);
        return productoActualizado;
    }

    public void eliminar(int id) {
        Producto3 prod = buscarPorId(id);
        if (prod != null) {
            listaProductos.remove(prod);
        }
    }
}